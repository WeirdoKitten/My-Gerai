...
**Landing `/` & direktori `/gerai` = pengecualian yang disengaja (2026-09-15, diperluas 2026-09-21).** Ini halaman *marketing*/direktori publik, bukan alur transaksi — tujuannya konversi (ajak daftar Lapak / lihat demo) atau menjelajah semua Gerai, jadi layar lebar sengaja dipakai penuh: bagian per bagian dalam `<section>` full-bleed (`border-t border-line` sebagai pemisah), masing-masing berisi container `mx-auto max-w-6xl px-5 sm:px-8`. Hero & seksi Asisten jadi `grid lg:grid-cols-2`, "Cara kerja" jadi `grid lg:grid-cols-3`, kartu fitur Pedagang/Pembeli jadi `grid lg:grid-cols-2` — semua tetap satu kolom di mobile (breakpoint `lg` saja). Header (`SiteHeader`) & footer (`SiteFooter`, `src/components/landing/`) dipakai bersama kedua halaman ini. Tetap dari komponen `ui/` (Card, Button/ButtonLink, Badge, ikon) + token yang sama, tanpa breakpoint layout di halaman lain. Judul seksi **tidak** pakai badge "eyebrow" di atasnya — tiap judul berdiri sendiri dengan naskah spesifik, bukan label generik berulang. **Statis, tanpa efek hover/scroll** (2026-09-21) — sempat dicoba animasi scroll-reveal + tilt-on-hover + parallax (`motion/react`), lalu dicabut semua atas permintaan User supaya halaman tetap ringan; satu-satunya sisa animasi berjalan adalah gumpalan gradien ambient di hero (`AmbientBlobs`, loop otomatis, bukan dipicu hover/scroll) dan `animate-bounce`/`animate-[shimmer_...]` CSS murni. Jangan tambah lagi animasi berbasis hover/scroll di halaman ini tanpa diminta User. **Pengecualian**: `PhoneMockup` (hero) sengaja jadi Client Component **interaktif** (klik) atas permintaan User — tombol "Tandai Diproses" beneran memajukan status Pesanan lewat `useState` (persis alur `nextMerchantStatus`), lalu setelah "Selesai" kartu hilang & Pesanan baru muncul (loop). Tetap tanpa `motion/react` (cuma `transition-opacity` CSS) — pemicunya klik User, bukan hover/scroll pasif, jadi tidak melanggar aturan di atas.
...
| Landing `/` & direktori `/gerai` | full-bleed per seksi, tiap seksi `max-w-6xl` — lihat pengecualian di atas |
...
- **Ikon header Pedagang** = **Notifikasi suara** (`SoundToggle`, `VolumeIcon`/`VolumeOffIcon`) + **QR Menu** (`QrIcon`) + **Profil** (`UserIcon`) di `headerAction` — kotak `size-9 border border-line rounded-control` (kelas dibagi lewat konstanta `HEADER_ICON_CLASS`, `src/components/merchant/header-icon-class.ts`). QR Menu & Profil tujuan **sesekali** (cetak QR sekali, atur profil jarang) → tidak makan slot bottom nav; toggle suara ditaruh sebaris karena keduanya kontrol status, bukan navigasi ke halaman lain.
...
- **Kartu Gerai publik (`MerchantShowcase`, dipakai landing & `/gerai`):** foto `aspect-[4/3]` dengan gradient gelap di bawah untuk keterbacaan teks putih, badge status buka/tutup (`Badge` tone `success`/`neutral` — "Buka"/"Tutup") di pojok kanan-atas foto, nama Lapak + kategori/Area + "Lihat Menu" di overlay bawah.
