...
- **Diketahui belum ditangani** (ditandai `/security-review`, sengaja ditunda ke Fase 5 — sama seperti rate-limiting checkout di Fase 2): belum ada rate-limiting/lockout di `loginMerchant`/`registerMerchant`/`loginAdmin`. Untuk MVP dengan basis pengguna kecil ini diterima sementara, tapi wajib ditambahkan sebelum basis pengguna membesar.
