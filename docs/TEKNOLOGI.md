...
| Animasi landing page (`/` saja) | **Motion** (`motion`, npm `motion/react` — penerus Framer Motion) | Izin User eksplisit (2026-09-15) untuk landing bergaya "Awwwards" (scroll-reveal, parallax, magnetic hover, tilt 3D) — level yang sulit dicapai CSS murni dengan mutu yang sama. Dipakai **hanya** lewat `src/components/landing/*` (semua `"use client"`), diimpor **hanya** oleh `src/app/page.tsx` — halaman lain (Pembeli, dashboard Pedagang, Admin) tetap nol dependency animasi tambahan (code-split per route oleh Next.js). Semua komponen menghormati `prefers-reduced-motion` (`useReducedMotion` dari Motion, di luar rule CSS global yang sudah ada). |
