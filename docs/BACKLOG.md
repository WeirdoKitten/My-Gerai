...
- [x] Pedagang: `OpenToggle` — ikon header tunggal (bukan pill+label, supaya muat di layar sempit bareng ikon QR/Profil/Pembayaran + nama Lapak; hijau=buka/merah=tutup, tap langsung toggle + Toast). Halaman baru `/dashboard/jadwal` (`OperatingHoursForm` 7 hari + status + tombol "Ikuti Jadwal Lagi") ditautkan dari kartu di `/dashboard/profil` (bukan ikon header terpisah — footprint header sudah penuh).
