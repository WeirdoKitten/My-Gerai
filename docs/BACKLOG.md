...
- **Bug pra-existing ditemukan (di luar lingkup fitur ini, belum diperbaiki)**: `seed.ts`/`seed-demo.ts` mengisi `photoUrl` Item dengan path statis (mis. `/img/menu/bakso.jpg`) yang tidak cocok `PRODUCT_PHOTO_URL_PATTERN` (`/uploads/products/<uuid>.<ext>`). Akibatnya `updateProduct` GAGAL validasi Zod ("Foto tidak valid.") untuk Item manapun yang masih pakai foto bawaan seed — Pedagang tidak bisa mengubah field apa pun (termasuk Harga Modal) pada Item itu sampai foto diganti/dihapus dulu. Berpotensi juga kena di produksi kalau `SEED_DEMO=true`. Perlu ditindaklanjuti terpisah (mis. `photoUrl: null` di seed, atau longgarkan validasi utk path `/img/**` bawaan).
