...
- [x] `src/app/page.tsx` — badge eyebrow di atas tiap judul seksi dihapus; seksi "Gerai Terdaftar" dipindah lebih awal (sebelum "Fitur"); ikon "Cara Kerja" tetap gaya asli (angka raksasa transparan di pojok kartu); jarak antara garis pembatas dan daftar fitur Pedagang/Pembeli final `mt-[52px]` (dikalibrasi beberapa kali: 36px → 48px → 64px → 56px → 52px); judul seksi "Asisten" final "Ada Asisten yang Bantu Kamu Jualan Lebih Pintar", kartu rekomendasinya jadi teks polos (judul + isi, tanpa ikon/badge — sempat dicoba badge label dan ikon lampu, keduanya dicabut lagi atas permintaan User); dua CTA penutup ("Daftarkan Lapak Sendiri" & "Ajak Pedagang Lain") tetap 2 seksi besar terpisah seperti semula; tanda em dash (—) di body copy diganti koma/titik dua.
